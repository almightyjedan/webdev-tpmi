...
function init() {
    ...
    fetch(`/hide/api/proxy/ceno-data?sheet=Data&range=A:A`)
        .then(res => res.text())
        .then(text => {
...
window.fetchCoefficients = function() {
    const baseUrl = '/hide/api/proxy/ceno-data';
    ...
    Promise.all([pN, pM, pR, pE]).then(responses => {
        ...
        handleQueryResponse({
            isError: () => false,
            getDataTable: () => parseGviz(responses[0])
        }, {
            isError: () => false,
            getDataTable: () => parseGviz(responses[1])
        }, {
            isError: () => false,
            getDataTable: () => parseGviz(responses[2])
        }, {
            isError: () => false,
            getDataTable: () => parseGviz(responses[3])
        });
    });
...
window.drawCurve = function(n, m, r, actX, actY) {
    ...
    var options = {
        ...
        series: {
            2: {
                lineDashStyle: [4, 4]
            }, // Garis Rate putus-putus
            3: {
                pointSize: 12,
                lineWidth: 0,
                pointShape: 'triangle',
                visibleInLegend: true,
                labelInLegend: 'Actual Point'
            } // Titik aktual bentuk segitiga
        },
        chartArea: {
            width: '92%',
            height: '80%',
...
window.updateAllSheets = function() {
    ...
    fetch(`${baseUrl}?sheet=Data&range=AD2:AH`)
        ...
        .then(text => {
            ...
            fetch('/hide/api/proxy/ceno-save', {
                method: 'POST',
                headers: {
